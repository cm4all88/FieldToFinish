...
& $csc /nologo /target:library /out:"$here\CleanupTestHarness.dll" "$here\CleanupTestHarness.cs" `
    /r:"$acad\accoremgd.dll" /r:"$acad\acdbmgd.dll" /r:"$acad\acmgd.dll" /r:"$bin\FieldCodes.dll" /r:"$bin\Newtonsoft.Json.dll" /r:System.Core.dll `
...
# The word APPROVED (not a title block tag such as APPROVEDBY) must never describe an exhibit.
$extra += if ($text -cmatch '\bAPPROVED\b') { "[FAIL] QA text says APPROVED" } else { "[PASS] QA text never says APPROVED" }
