# Dumps block definitions (entities, attribute definitions and their field codes) from copies of real drawings.
#   dump-blocks.ps1 -Drawing <path> -Blocks "A;B;C" -Name <report name> [-Out <folder>]
param([string]$Drawing, [string]$Blocks, [string]$Name, [string]$Out = 'C:\dev\FTF-RealWorld\dumps')
...
Set-Content -LiteralPath $scr -Encoding ascii -Value @('NETLOAD "' + "$here\StandardsDump.dll" + '"', 'BLKDUMP', $Blocks, (Join-Path $Out ($Name + '.txt')), '')
